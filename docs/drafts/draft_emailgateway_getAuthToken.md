...
| **V-001** | **Mandatory Field Validation**                | Mandatory              | `keys` must not be null or empty                                           | Reject request if missing or empty                      |
| **V-001** | **Mandatory Field Validation**                | Mandatory              | Each key in the `keys` array must be non-empty                             | Reject request if any key is empty                      |
| **V-002** | **Correlation ID Validation**                 | Null Check             | `correlationID` is null                                                    | Generate a new GUID; no validation error                |
| **V-002** | **Response Code Validation**                  | Null Check             | `responseCode` is null                                                     | Treat `responseCode` as **500 (Internal Server Error)** |
| **V-003** | **Access Token Validation**                   | Empty / Null Check     | `access_token` returned in the response is null or empty                   | Return error: **"Token is missing or empty"**           |
| **V-003** | **Response Body Validation**                  | Empty Check            | `responseBody` is empty                                                    | Return error: **"Empty response body received"**        |
| **V-004** | **Request Timestamp Validation**              | Format Validation      | `requestTimestamp`, if present, must match `yyyy-MM-dd HH:mm:ss.SSS`       | Reject/flag invalid timestamp format                    |
| **V-004** | **Response Code Validation**                  | Format Validation      | `responseCode`, if present, must be a numeric string, e.g. `200`           | Reject/flag invalid response code format                |
| **V-005** | **Regex Validation**                          | Regex                  | No specific regex validation is required for input fields                  | Values are validated through service logic              |
| **V-006** | **Microsoft Graph Token Base URL Validation** | Lookup / Configuration | `MICROSOFT_GRAPH_TOKEN_BASE_URL` must be present and start with `https://` | Reject/flag invalid configuration                       |
| **V-006** | **Microsoft Graph Client ID Validation**      | Lookup / Configuration | `MICROSOFT_GRAPH_TOKEN_CLIENT_ID` must not be empty                        | Reject/flag invalid configuration                       |
| **V-006** | **Microsoft Graph Tenant ID Validation**      | Lookup / Configuration | `MICROSOFT_GRAPH_TOKEN_TENANT_ID` must not be empty                        | Reject/flag invalid configuration                       |
| **V-007** | **Duplicate Token / Cache Handling**          | Duplicate Check        | Token already exists in cache with the same ke                             |                                                         |
...
var options = {
  'method': 'POST',
  'url': 'MICROSOFT_GRAPH_TOKEN_BASE_URL/MICROSOFT_GRAPH_TOKEN_TENANT_ID/oauth2/v2.0/token',
  'headers': {
    'Content-Type': 'application/x-www-form-urlencoded'
  },
  form: {
    'client_id': 'MICROSOFT_GRAPH_TOKEN_CLIENT_ID',
    'client_secret': 'MICROSOFT_GRAPH_TOKEN_CLIENT_SECRET',
...
# 13. REST API Design for Spring Boot

Draft: No Need of having an API Endpoint
